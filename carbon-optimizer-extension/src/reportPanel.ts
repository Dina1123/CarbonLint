...
export class ReportPanel {
    ...
    if (this.panel) {
      ...
      this.panel.webview.onDidReceiveMessage((message) => {
        if (message.command === 'ready') {
          ...
          void this._copyCode(message.code as string);
        } else if (message.command === 'apply') {
          void this._applyCode(message.code as string);
        }
      });
  ...
  renderReport(report: Report, fileName: string): string {
  ...
  private _readyHtml(fileName: string): string {
    return `
  ...
  private _loadingHtml(fileName: string): string {
    return `
      <div class="file-pill" style="margin-bottom:16px">🐍 ${this._escapeHtml(fileName)}</div>
      <div class="state-screen">
  ...
  private _errorHtml(message: string, fileName: string): string {
    return `
      <div class="file-pill" style="margin-bottom:16px">🐍 ${this._escapeHtml(fileName)}</div>
  ...
  private _humanizeEnergy(kwh: number): string {
    const ledSeconds = kwh / 2.778e-6; // 10 W LED = 2.778e-6 kWh/s
    if (ledSeconds < 0.001) return `${(ledSeconds * 1e6).toFixed(1)} µs of a 10 W LED bulb`;
    if (ledSeconds < 1)     return `${(ledSeconds * 1000).toFixed(1)} ms of a 10 W LED bulb`;
    if (ledSeconds < 60)    return `${ledSeconds.toFixed(2)} s of a 10 W LED bulb`;
    return `${(ledSeconds / 60).toFixed(2)} min of a 10 W LED bulb`;
  }

  private _humanizeCO2(grams: number): string {
    const meters = grams / 0.12; // avg car ≈ 120 g CO₂/km → 0.12 g/m
    if (meters < 0.01)  return `${(meters * 100).toFixed(2)} mm driven by car`;
    if (meters < 1)     return `${(meters * 100).toFixed(1)} cm driven by car`;
    if (meters < 1000)  return `${meters.toFixed(1)} m driven by car`;
